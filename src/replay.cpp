...
void setFrames(SlippiReplay &s, int32_t nframes) {
  ...
  for(unsigned i = 0; i < 4; ++i) {
    ...
    // memset(s.player[i].frame, 0, (nframes+123)*sizeof(SlippiFrame));
  }
}
...
std::string replayAsJson(SlippiReplay s) {
  ...
  for(unsigned p = 0; p < 4; ++p) {
    ...
    ss << JUIN(4,"ext_char_id" ,s.player[p].ext_char_id)      << ",\n";
    ss << JUIN(4,"player_type" ,s.player[p].player_type)      << ",\n";
    ss << JUIN(4,"start_stocks",s.player[p].start_stocks)     << ",\n";
    ss << JUIN(4,"color"       ,s.player[p].color)            << ",\n";
    ss << JUIN(4,"team_id"     ,s.player[p].team_id)          << ",\n";
    ss << JUIN(4,"dash_back"   ,s.player[p].dash_back)        << ",\n";
    ss << JUIN(4,"shield_drop" ,s.player[p].shield_drop)      << ",\n";
    ss << JSTR(4,"tag"         ,escape_json(s.player[p].tag)) << ",\n";
    ...
    if (s.player[p].player_type == 3) {
      ...
      for(unsigned f = 0; f < total_frames; ++f) {
        ...
        if (CHANGED(frame))
        ss << JINT(6,"frame"         ,s.player[p].frame[f].frame)        << ",\n";
        if (CHANGED(follower))
        ...
        ss << JUIN(6,"player"        ,s.player[p].frame[f].player)       << "\n";
